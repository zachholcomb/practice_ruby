...
class LinkedListTest < Minitest::Test
  def test_it_works_for_one_element
    element = Element.new(1)
    assert_equal 1, element.datum
  end

  def test_element_can_hold_different_value
    element = Element.new(10)
    assert_equal 10, element.datum
  end

  def test_elment_to_the_next_is_nil
    element = Element.new(1)
  ...
  def test_list_can_add_element
    first  = Element.new(1)
    list = SimpleLinkedList.new
    assert_equal list, list.push(first)
  end
  ...
  def test_list_created_from_array_still_made_up_of_elements
   ...
   def test_list_in_place_reverse_are_the_same_elements
    ...
    assert_equal 1, list.pop.datum
    assert_equal 2, list.pop.datum
  end
